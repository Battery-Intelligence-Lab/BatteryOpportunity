function plt_profit_vs_interest_daily(cal, cyc, both)
plt_common;
...
N_CC = length(both.all.CC_list_daily);
my_CC_to_plot = [0, 0.01, 0.02, 0.03, 0.05, 0.08, 0.12, 0.2];
iCC_to_plot = interp1(both.all.CC_list,1:length(both.all.CC_list_daily), my_CC_to_plot);
CC_to_plot = both.all.CC_list(iCC_to_plot);

colors= flipud(viridis(length(CC_to_plot)));
icolor = 1;
for iii=iCC_to_plot
    plot(both.all.lambda_cal', both.all.PI_list_daily(:,iii),lw{:},'color',colors(icolor,:)); hold on;
    icolor = icolor+1;
end

lin_curve = [1e-3, max(both.all.PI_list_daily(:,1))]*1.2;
